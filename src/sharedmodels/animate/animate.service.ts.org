...
export class AnimateService {
  ...
  // private _audioProfile: AudioProfile;
  private _segments: Segment[];
  private _standpunkte: Position[];

  constructor(private store: Store<any>, private audioService: AudioService) {
    this.store.select('segments').subscribe( (segs: Segment[]) => {
      this._segments = segs;
      this._standpunkte = undefined;
      this.onStop();
    });
    // store.select('audioProfile').subscribe( (af: AudioProfile) => this._audioProfile = af);
    this.store.select('animationRunning').subscribe( (v: boolean) => this._videoRunning = v);
  }
  ...
  public onRun(): void {
    ...
    this.audioService.playAb(this.delta);
  ...
  public positionPferd(t: number): void {
    ...
    if (t >= 0) {
      ...
      if (pos) {
        let newPos: Position = {x: pos.x, y: pos.y, direction: pos.direction, gang: st.g,
                                  rotation: pos.rotation, hand: pos.hand, stellung: st.stellung};
  ...
  private animate: Function = (offset: number = 0, backwards: boolean = false) => {
    ...
    const animateme: FrameRequestCallback = () => {
      ...
      if (this.delta <= dauer && this.delta >= 0) {
        ...
        if (pos) {
          let newPos: Position = {x: pos.x, y: pos.y, direction: pos.direction, gang: st.g,
                                    rotation: (pos.rotation || 0) + this.schwankung, hand: pos.hand, stellung: st.stellung};
          this.store.dispatch({type: 'STANDPUNKTE_ADD_ITEMS', payload: [newPos]});
        }
        this.store.dispatch({type: 'SEGMENTS_SELECT_ITEM', payload: st.seg});
        if (this._videoRunning) {
  ...
  private standpunktNachStrecke(entfernung: number): Position {
    if (!this._standpunkte || this._standpunkte.length === 0)
      return undefined;
    ...
    if (index === this._standpunkte.length - 1) {
      ...
      for (let i: number = index; i < this._standpunkte.length; i++) {
        ...
        distanz += vector_distance(this._standpunkte[i - 1].position, this._standpunkte[i].position);
      }
    }
    return s;
  }
  private triggerSchwankung() {
